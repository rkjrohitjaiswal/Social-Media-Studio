...
export default function BrandStudioPage() {
  ...
  useEffect(() => {
    async function loadBrand() {
      try {
        const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/brand");
  ...
  const handleLogoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/brand/logo", {
        method: "POST",
        body: formData,
      });
      const result = await res.json();
  ...
  const handleRemoveLogo = async () => {
    setIsUploadingLogo(true);
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/brand/logo", { method: "DELETE" });
      const result = await res.json();
