...
import { QueueCopyState } from "@/lib/queue-types";
import { QueueQualityState, ReviewEventState, AssetApprovalRecord } from "@/lib/queue-types";
import { AccountState, PublicationState } from "@/lib/queue-types";
...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    ...
    async function loadApprovalData() {
      try {
        const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/campaigns/demo-campaign-1/copy");
        ...
        const qualRes = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/campaigns/demo-campaign-1/quality");
        ...
        const appRes = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/campaigns/demo-campaign-1/approvals");
        ...
        const igRes = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/integrations/instagram");
  ...
  const selectCopy = (copy: QueueCopyState) => {
    ...
    setSelectedVersionNum(copy.currentVersionNumber || 1);
  };
  ...
  const handleSaveCopyEdits = async () => {
    ...
    try {
      ...
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/campaigns/demo-campaign-1/copy/${activeCopy.id}`, {
        method: "PUT",
  ...
  const handleApprove = async () => {
    if (!activeCopy) return;
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/campaigns/demo-campaign-1/approvals/${activeCopy.generatedAssetId}`, {
        method: "POST",
      });
      const json = await res.json();
  ...
  const handleRequestChanges = async () => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/campaigns/demo-campaign-1/approvals/${activeCopy.generatedAssetId}/request-changes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleReject = async () => {
    if (!activeCopy) return;
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/campaigns/demo-campaign-1/approvals/${activeCopy.generatedAssetId}/reject`, {
        method: "POST",
      });
      const json = await res.json();
      if (json.success && json.approval) {
  ...
  const handleRegenerateTextCopy = async () => {
    ...
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/campaigns/demo-campaign-1/copy/${activeCopy.id}/regenerate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          generatedAssetId: activeCopy.generatedAssetId,
  ...
  const handleExecutePublishing = async () => {
    ...
    try {
      ...
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + `/api/campaigns/demo-campaign-1/publish/${activeCopy.generatedAssetId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/10 pt-4 text-xs">
                  {qualityAssessment.issues.length > 0 && (
                    <div className="p-3 rounded-2xl bg-[#a84b4b]/10 border border-[#a84b4b]/30 space-y-1 text-[#f5f4f0]">
                      ...
                      <ul className="list-disc list-inside text-[11px] space-y-1 text-[#9e9d98]">
                        {qualityAssessment.issues.map((issue: string, idx: number) => (
                          <li key={idx}>{issue}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {qualityAssessment.recommendations.length > 0 && (
                    <div className="p-3 rounded-2xl bg-[#c5a059]/10 border border-[#c5a059]/30 space-y-1 text-[#f5f4f0]">
                      ...
                      <ul className="list-disc list-inside text-[11px] space-y-1 text-[#9e9d98]">
                        {qualityAssessment.recommendations.map((rec: string, idx: number) => (
                          <li key={idx}>{rec}</li>
                        ))}
                      </ul>
                    </div>
