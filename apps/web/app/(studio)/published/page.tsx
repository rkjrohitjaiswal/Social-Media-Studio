...
import { PublicationState } from "@/lib/queue-types";
...
export default function PublishedPage() {
  ...
  useEffect(() => {
    ...
    async function loadPublications() {
      try {
        const res = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/campaigns/demo-campaign-1/publish");
        const json = await res.json();
        if (isMounted && json.success && json.publications) {
          setPublications(json.publications);
        }

        const resMedia = await fetch((process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") + "/api/analytics/media?limit=50");
              ...
              <div key={pub.id} className="glass-card p-6 rounded-3xl space-y-4 border-[#c5a059]/30">
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div className="flex flex-wrap gap-1 pt-1">
                    {pub.hashtagsSnapshot?.map((tag: string, idx: number) => (
