...
const content = (caption: string, contentType = "GENERAL_POST" as const) => ({
  id: "content-1",
  workspaceId: "ws-1",
  platform: "X" as const,
  contentType,
...
describe("XProvider", () => {
  ...
  it("adds affiliate disclosure when missing", async () => {
    const result = await provider.publish({ workspaceId: "ws-1", platform: "X", account, content: content("Check this useful product", "AFFILIATE_PRODUCT" as never) });
    expect(result.success).toBe(true);
  });
