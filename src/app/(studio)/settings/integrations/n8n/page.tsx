...
export default function N8nSettingsPage() {
  ...
  const fetchDeliveries = async (integrationId: string) => {
    ...
    try {
      const res = await fetch(`/api/integrations/n8n/${integrationId}/deliveries`);
      const json = await res.json();
      if (json.success) {
  ...
  const handleToggleEnabled = async (integration: N8nIntegration) => {
    try {
      await fetch(`/api/integrations/n8n/${integration.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteIntegration = async (id: string) => {
    ...
    try {
      await fetch(`/api/integrations/n8n/${id}`, { method: "DELETE" });
      fetchIntegrations();
    } catch {
      // Ignore delete error
    }
  ...
  const handleTestWebhook = async (id: string) => {
    setTestingId(id);
    try {
      const res = await fetch(`/api/integrations/n8n/${id}/test`, { method: "POST" });
      const json = await res.json();
      if (json.success) {
  ...
  const handleRetryDelivery = async (deliveryId: string) => {
    try {
      const res = await fetch(`/api/integrations/n8n/deliveries/${deliveryId}/retry`, {
        method: "POST",
      });
      const json = await res.json();
