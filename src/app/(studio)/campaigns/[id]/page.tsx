...
import { QueueRunState, QueueJobState } from "@/lib/queue/generation-worker";
...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    const fetchRunStatus = async () => {
      try {
        const res = await fetch(`/api/campaigns/${campaignId}/generate/run`);
    ...
    const eventSource = new EventSource(`/api/campaigns/${campaignId}/generate/events`);
  ...
  const handleRetryJob = async (jobId: string) => {
    ...
    try {
      const res = await fetch(`/api/campaigns/${campaignId}/generate/retry`, {
        method: "POST",
